...
LCD_WR_REG(0x36); 
if(DERECTION==0)LCD_WR_DATA8(0x00);
else if(DERECTION==1)LCD_WR_DATA8(0xC0);
else if(DERECTION==2)LCD_WR_DATA8(0x70);
...
{  
	...
	u8 *temp;
	u16 sizex1,size3;
	if(sizex==16&&sizey==16)temp=Hzk16;//选择16x16字号
	if(sizex==32&&sizey==32)temp=Hzk32;//选择32x32字号
//	if(sizex==xx&&sizey==xx)temp=Hzkxx;//选择XXxXX字号
	
	sizex1=sizex/8;
	if(sizex%8!=0)sizex1++;
  LCD_Address_Set(x,y,x+sizex1*8-1,y+sizey-1); //设置一个汉字的区域
	size3=sizex1*sizey;//一个汉字所占的字节
	temp+=index*size3;//写入的起始位置
	for(j=0;j<size3;j++)
	{
		for(i=0;i<8;i++)
		{
